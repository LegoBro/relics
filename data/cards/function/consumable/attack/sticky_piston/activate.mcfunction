...
playsound minecraft:block.piston.contract master @a[tag=id] ~ ~ ~ 10 1 1
...
execute at @n[type=armor_stand,tag=hovered.slot,tag=filled,tag=id,distance=..100] as @e[tag=attackable,tag=id,limit=1,sort=nearest] run function cards:push
