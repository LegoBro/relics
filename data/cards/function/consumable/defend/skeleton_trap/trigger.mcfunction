...
execute as @e[type=armor_stand,tag=board,tag=id,tag=!draw_pile,tag=!discard_pile,tag=!filled,tag=!blocked,tag=!block,limit=2,sort=random] at @s run tag @s add skeleton_trap.summon
...
execute if entity @s[tag=skeleton_trap.2] if entity @a[tag=id,tag=turn,tag=player.1] as @e[type=skeleton_horse,tag=new,tag=id] at @s run function card:helper/not_friendly
execute if entity @s[tag=skeleton_trap.1] if entity @a[tag=id,tag=turn,tag=player.2] as @e[type=skeleton_horse,tag=new,tag=id] at @s run function card:helper/not_friendly
