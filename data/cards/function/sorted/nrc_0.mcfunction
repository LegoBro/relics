## Sorted view of cards
function cards:consumable/attack/anvil/insert
function cards:consumable/buff/aqua_affinity/insert
function cards:entity/overworld/armorsmith/insert
function cards:consumable/attack/arrow_storm/insert
function cards:entity/overworld/axolotl/insert
function cards:entity/nether/baby_magma_cube/insert
function cards:entity/overworld/bee/insert
function cards:consumable/defend/bee_hive/insert
function cards:consumable/defend/bee_nest/insert
function cards:entity/nether/blaze/insert
function cards:consumable/attack/blaze_strike/insert
function cards:entity/overworld/bogged/insert
function cards:consumable/buff/bread/insert
function cards:entity/overworld/butcher/insert
function cards:entity/overworld/camel/insert
function cards:entity/overworld/camel_husk/insert
function cards:entity/overworld/cave_spider/insert
function cards:entity/overworld/charged_creeper/insert
function cards:consumable/defend/cherry_sapling/insert
function cards:entity/overworld/chicken/insert
function cards:consumable/attack/chorus_fruit/insert
function cards:entity/overworld/cleric/insert
function cards:consumable/defend/cobblestone/insert
function cards:consumable/defend/cobweb_trap/insert
function cards:entity/overworld/copper_golem/insert
function cards:entity/overworld/creeper/insert
function cards:consumable/attack/critical_hit/insert
execute positioned ~1 ~ ~ run function cards:entity/overworld/cub/insert
execute positioned ~1 ~ ~ run function cards:consumable/buff/curse_of_binding/insert
execute positioned ~1 ~ ~ run function cards:consumable/buff/diamond_armor/insert
execute positioned ~1 ~ ~ run function cards:consumable/defend/diorite/insert
execute positioned ~1 ~ ~ run function cards:entity/overworld/dolphin/insert
execute positioned ~1 ~ ~ run function cards:consumable/attack/dragons_breath/insert
execute positioned ~1 ~ ~ run function cards:entity/overworld/drowned/insert
execute positioned ~1 ~ ~ run function cards:entity/overworld/elder_guardian/insert
execute positioned ~1 ~ ~ run function cards:consumable/buff/elytra/insert
execute positioned ~1 ~ ~ run function cards:consumable/buff/emerald/insert
execute positioned ~1 ~ ~ run function cards:consumable/buff/enchanted_golden_apple/insert
execute positioned ~1 ~ ~ run function cards:consumable/defend/enchantment_table/insert
execute positioned ~1 ~ ~ run function cards:entity/end/end_crystal/insert
execute positioned ~1 ~ ~ run function cards:entity/end/ender_dragon/insert
execute positioned ~1 ~ ~ run function cards:consumable/attack/ender_pearl/insert
execute positioned ~1 ~ ~ run function cards:entity/end/enderman/insert
execute positioned ~1 ~ ~ run function cards:entity/end/endermite/insert
execute positioned ~1 ~ ~ run function cards:entity/overworld/evoker/insert