...
#### PRINT LORE
    ...
    give_command = "give @s carrot_on_a_stick{gui:False,display:{" + lore + ",Name:'[{\"translate\":\"card." + card["name"] + "\",\"italic\":\"false\",\"color\":\"" + card["color"] + "\"}]'},HideFlags:127,Unbreakable:1b,CustomModelData:" + str(card["model_id"]) + ",card:{category:\"" + card["type"] + "\",element:\"" + card["element"] + "\",name:\"" + card["name"] + "\",cost:" + str(card["cost"]) + ",placement:" + str(card["placement"]) + "}} 1"
    ...
    unlock_command = "give @s carrot_on_a_stick{gui:False,display:{" + lore + ",Name:'{\"translate\":\"card." + card["name"] + "\",\"italic\":\"false\",\"color\":\"" + card["color"] + "\"}'},HideFlags:127,Unbreakable:1b,CustomModelData:" + str(card["model_id"]) + ",card:{category:\"" + card["type"] + "\",element:\"" + card["element"] + "\",name:\"" + card["name"] + "\",cost:" + str(card["cost"]) + ",placement:" + str(card["placement"]) + "}} 1"
...
for type in types:
    ...
    for subtype in subtypes:
        ...
        for card in cards:
            ...
            collection.append(createCard(f"./yaml/{type}/{subtype}/{card}"))
            
            pass


        use_card_command = "#subtypes in card use tree (generated)"
        hover_card_command = "#subtypes in card hover tree (generated)"

        for item in card_list:
            item = item.replace(".yaml","")
            use_card_command += f'\nexecute if entity @s[nbt={{SelectedItem:{{tag:{{card:{{name:"{item}"}}}}}}}}] run function cards:{type}/{subtype}/{item}/use'
            hover_card_command += f'\nexecute if entity @s[nbt={{SelectedItem:{{tag:{{card:{{name:"{item}"}}}}}}}}] run function cards:{type}/{subtype}/{item}/hover'
            #print(item)



        generator.writeFunction(f"./{type}/{subtype}/use",use_card_command)
        generator.writeFunction(f"./{type}/{subtype}/hover",hover_card_command)
    ...
    for item in subtype_list:
        ...
        hover_subtype_command += f'\nexecute if entity @s[nbt={{SelectedItem:{{tag:{{card:{{element:"{item}"}}}}}}}}] run function cards:{type}/{item}/hover'
...
use_type_command = "#Origin point of card tree (generated)"
hover_type_command = "#Origin point of card tree (generated)"
for item in type_list:
    use_type_command += f'\nexecute if entity @s[nbt={{SelectedItem:{{tag:{{card:{{category:"{item}"}}}}}}}}] run function cards:{item}/use'
    hover_type_command += f'\nexecute if entity @s[nbt={{SelectedItem:{{tag:{{card:{{category:"{item}"}}}}}}}}] run function cards:{item}/hover'
    #print(item)
use_type_command += "\nscoreboard players reset @s rightClick"
use_type_command += "\nscoreboard players add @s play.card.turn 1"

generator.writeFunction("./use",use_type_command)

generator.writeFunction("./hover",hover_type_command)
