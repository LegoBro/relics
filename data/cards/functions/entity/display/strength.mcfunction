...
scoreboard players operation #display var = #strength var
...
summon area_effect_cloud ~ ~2.5 ~ {CustomNameVisible:1b,Particle:"ash",Duration:5,Tags:["get_id","id","rename"]}
scoreboard players operation @e[tag=get_id] id = game.id var
execute as @e[type=area_effect_cloud,tag=rename,tag=id,limit=1,sort=nearest,tag=get_id] run data modify entity @s CustomName set from block 0 1 1 Text1
